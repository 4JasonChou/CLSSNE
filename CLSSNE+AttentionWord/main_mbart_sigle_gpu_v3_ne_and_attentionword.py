...
def CreateTrainingDataLoader( tokenizer , envBatchSize , inputCsvNameList) :
    ...
    for inputCsvName in inputCsvNameList :
        ...
        for index,rowData in inputCsv.iterrows() :
            ...
            tmpContext = NameEntitysHandler(rowData['name_entity']) + '[CL]' + NameEntitysHandlerWithSize(rowData['ne_keyword']) + '[CL]' + rowData["context"]
...
# Change AttentionWord Size
def NameEntitysHandlerWithSize(neListStr) :
    neHints_tmp = ast.literal_eval(neListStr)
    if len(neHints_tmp) >= 5 : 
        neHints_tmp = neHints_tmp[:5]
    neHints = ','.join(neHints_tmp)
    return neHints
...
if __name__ == "__main__":
    ...
    # Init Logger : 
    logging_file = "mBartFineturn_attentionWord_5.log".format(modelName)
    logging.basicConfig(filename=logging_file, level=logging.INFO) 
    logging.info( TimeMsg( "ModelTask:{}, Traingset:{}".format(modelName,trainingDataFileName) ) )
    logging.info( TimeMsg( "Info: Predict important name-entity and summary." ) )
