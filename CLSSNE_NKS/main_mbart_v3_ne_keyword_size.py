...
def CreateTrainingDataLoader( tokenizer , envBatchSize , inputCsvNameList) :
    ...
    for inputCsvName in inputCsvNameList :
        ...
        for index,rowData in inputCsv.iterrows() :
            ...
            # input - context , 輸入 : 摘要長度 + [SIZE] + 主體NE(s) + [CL] + 輔助NE(s) + [CL] + 內文 ( 2 + 10 + 10 + 512  )
            tmpContext = str(len(unitTextTemp_Zh)) + '[SIZE]' + NameEntitysHandler(rowData['new_name_entity']) + '[CL]' + NameEntitysHandlerWithSize(rowData['ne_keyword']) + '[CL]' + rowData["context"]
            inputContext = tokenizer(tmpContext, max_length=534, pad_to_max_length=True, return_tensors='pt')
...
if __name__ == "__main__":
    ...
    # Env Par :
    trainingDataFileName = ['../Dataset/EN2ZHSUM_train_v3_ne_part1_keyword_emptyfix.csv',
                            '../Dataset/EN2ZHSUM_train_v3_ne_part2_keyword_emptyfix.csv',
                            '../Dataset/EN2ZHSUM_train_v3_ne_part3_keyword_emptyfix.csv']
    testingDataFileName = '../Dataset/EN2ZHSUM_test_v3_ne_keyword_fix.csv' 
    modelName = 'Summary_fzpar_en2zh_v3_ne_keyword_size'
    mEnv_batch_szie = 2 
    mTrainEpoch = 10
    accumulation_step = 8
    ...
    # Init Logger : 
    logging_file = "mBartFineturn_vNKS.log".format(modelName)
    logging.basicConfig(filename=logging_file, level=logging.INFO) 
    logging.info( TimeMsg( "ModelTask:{}, Traingset:{}".format(modelName,trainingDataFileName) ) )
    logging.info( TimeMsg( "Info: Predict important name-entity and keyword in summarization v2 with sentence size controll." ) )
    ...
    special_tokens_dict['additional_special_tokens'].append('[SIZE]')
    ...
    mBartTokenizer.save_pretrained("tokenizer_vNKS/".format(modelName))
    ...
    # 創建訓練資料 : MaxLen : 1024 ( OOM ) , 只能使用 Len : 534  ( MainNe + CL + SubNe + CL +Context )
    ...
    for epoch in range(mTrainEpoch):
        ...
        mBartModel.save_pretrained("Checkpoint_vNKS/point_{}".format(str(epoch)))
        ...
        for index , row in testingPd.iterrows() :
            ...
            tmpContext = str(len(tmplabelZh)) + '[SIZE]' + NameEntitysHandler(row['name_entity_fix']) + '[CL]' + NameEntitysHandlerWithSize(row['ne_keyword']) + '[CL]' + row["context"]
            tmpInput = mBartTokenizer( tmpContext, max_length=534, return_tensors='pt' )
            tmpInput.to(mDevice)
