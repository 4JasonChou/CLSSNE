...
def CreateTrainingDataLoader( tokenizer , envBatchSize , inputCsvNameList) :
    ...
    for inputCsvName in inputCsvNameList :
        ...
        for index,rowData in inputCsv.iterrows() :
            ...
            # input - context , 參考 GetInputContext Method
            tmpContext = GetInputContext(rowData['name_entity'],
                                         rowData['new_name_entity'],
                                         rowData['name_entity_zh'],
                                         rowData['ne_keyword'],
                                         rowData['context'])
            inputContext = tokenizer(tmpContext, max_length=576, pad_to_max_length=True, return_tensors='pt')
...
def ImpContextHandler( context, impStrList ) :
    impList = ast.literal_eval(impStrList)
    if len(impList) <= 0 :
        return context
    newContext = context
    for i in range(len(impList)) :
        if i > 2 :
            break
        imp_index = newContext.find(impList[i])
        newContext = newContext[:imp_index] + '[IMP]' + newContext[imp_index:]
    return newContext
...
def NameEntitysHandlerWithSize(neListStr) :
    neHints_tmp = ast.literal_eval(neListStr)
...
def GetInputContext( nes, nes_fix, nes_zh, nes_keyword, context) :
    # Version - INE + INE_Translation + Keyword + context , 預計
    if nes == '[]' : 
        tmpContext = '[NE]' + NameEntitysHandler(nes_fix) + '[TS][KE]' + NameEntitysHandlerWithSize(nes_keyword) + '[SU]' + context
        return tmpContext
    
    en_nes = nes.split(',')
    zh_nes = nes_zh.split(',')
    if len(en_nes) == len(zh_nes) :
        tmpContext = '[NE]' + NameEntitysHandler(nes) + '[TS]' + NameEntitysHandler(nes_zh) + '[KE]' + NameEntitysHandlerWithSize(nes_keyword) + '[SU]' + context
        return tmpContext
...
if __name__ == "__main__":
    ...
    # Env Par :
    trainingDataFileName = ['../Dataset/EN2ZHSUM_train_v4_ne_part1_keyword_emptyfix.csv',
                            '../Dataset/EN2ZHSUM_train_v4_ne_part2_keyword_emptyfix.csv',
                            '../Dataset/EN2ZHSUM_train_v4_ne_part3_keyword_emptyfix.csv']
    testingDataFileName = '../Dataset/EN2ZHSUM_test_v4_ne_keyword_fix.csv' 
    modelName = 'Summary_fzpar_en2zh_v4_ne_zh_keyword'
    mEnv_batch_szie = 2 
    mTrainEpoch = 10
    accumulation_step = 8
    ...
    # Init Logger : 
    logging_file = "mBartFineturn.log".format(modelName)
    logging.basicConfig(filename=logging_file, level=logging.INFO) 
    logging.info( TimeMsg( "ModelTask:{}, Traingset:{}".format(modelName,trainingDataFileName) ) )
    logging.info( TimeMsg( "Info: Predict important name-entity with tranlation and keyword in summarization." ) )
    ...
    special_tokens_dict['additional_special_tokens'].append('[NE]')
    special_tokens_dict['additional_special_tokens'].append('[TS]')
    special_tokens_dict['additional_special_tokens'].append('[KE]')
    special_tokens_dict['additional_special_tokens'].append('[SU]')
    ...
    mBartTokenizer.save_pretrained("tokenizer/".format(modelName))
    ...
    # 創建訓練資料 : MaxLen : 1024 ( OOM ) , 只能使用 Len : 576 
    ...
    for epoch in range(mTrainEpoch):
        ...
        mBartModel.save_pretrained("Checkpoint/point_{}".format(str(epoch)))
        ...
        for index , row in testingPd.iterrows() :
            tmpContext = GetInputContext(row['name_entity'],
                                         row['name_entity_fix'],
                                         row['name_entity_zh'],
                                         row['ne_keyword'],
                                         row['context'])
            tmplabelZh = CsvSummaryZhHandler(row['zh_refs'])
            tmpInput = mBartTokenizer( tmpContext, max_length=576, return_tensors='pt' )
            tmpInput.to(mDevice)
