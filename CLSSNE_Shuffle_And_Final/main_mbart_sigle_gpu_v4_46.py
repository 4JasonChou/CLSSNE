...
def CreateTrainingDataLoader( tokenizer , envBatchSize , inputCsvNameList) :
    ...
    for inputCsvName in inputCsvNameList :
        ...
        for index,rowData in inputCsv.iterrows() :
            ...
            tmpContext = NameEntitysHandler(str(rowData['name_entity'])) + '[CL]' + NameEntitysHandlerWithSize(str(rowData['ne_keyword'])) + '[CL]' + tmpContext
...
if __name__ == "__main__":
    ...
    # Env Par :
    trainingDataFileName = ['../Dataset/EN2ZHSUM_train_v3_ne_part1_keyword_emptyfix_upset_46change.csv',
                            '../Dataset/EN2ZHSUM_train_v3_ne_part2_keyword_emptyfix_upset_46change.csv',
                            '../Dataset/EN2ZHSUM_train_v3_ne_part3_keyword_emptyfix_upset_46change.csv']
    ...
    # Init Logger : 
    logging_file = "mBartFineturn_vINEPlus_46c_60p.log".format(modelName)
    logging.basicConfig(filename=logging_file, level=logging.INFO) 
    logging.info( TimeMsg( "ModelTask:{}, Traingset:{}".format(modelName,trainingDataFileName) ) )
    logging.info( TimeMsg( "Info: Predict important name-entity, keyword and summary. with 4:6 UpsetContext with 60%" ) )
    ...
    for epoch in range(mTrainEpoch):
        ...
        mBartModel.save_pretrained("Checkpoint_vINEPlus_Final/point_{}".format(str(epoch)))
