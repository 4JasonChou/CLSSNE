...
def CreateTrainingDataLoader( tokenizer , envBatchSize , inputCsvNameList) :
    ...
    for inputCsvName in inputCsvNameList :
        ...
        for index,rowData in inputCsv.iterrows() :
            ...
            tmpArt = str(rowData["context"])
            useUpsetContext = GetRandomWith20Persent()
            if useUpsetContext : 
                tmpArt = str(rowData["upset_context"])
                
            # input - context , 輸入 : 主體NE(s) + [CL] + 輔助NE(s) + [CL] + 內文 ( 10 + 10 + 512  )
            tmpContext = NameEntitysHandler(str(rowData['name_entity'])) + '[CL]' + NameEntitysHandlerWithSize(str(rowData['ne_keyword'])) + '[CL]' + tmpArt
            inputContext = tokenizer(tmpContext, max_length=532, pad_to_max_length=True, return_tensors='pt')
...
if __name__ == "__main__":
    ...
    # Env Par :
    trainingDataFileName = ['../Dataset/EN2ZHSUM_train_v3_ne_part1_keyword_emptyfix_upset.csv',
                            '../Dataset/EN2ZHSUM_train_v3_ne_part2_keyword_emptyfix_upset.csv',
                            '../Dataset/EN2ZHSUM_train_v3_ne_part3_keyword_emptyfix_upset.csv']
    testingDataFileName = '../Dataset/EN2ZHSUM_test_v3_ne_keyword_fix.csv' 
    modelName = 'Summary_fzpar_en2zh_v3_ne_and_keyword_input'
    mEnv_batch_szie = 2 
    mTrainEpoch = 20
    accumulation_step = 8
    ...
    # Init Logger : 
    logging_file = "mBartFineturn_vINEPulsAnd37.log".format(modelName)
    logging.basicConfig(filename=logging_file, level=logging.INFO) 
    logging.info( TimeMsg( "ModelTask:{}, Traingset:{}".format(modelName,trainingDataFileName) ) )
    logging.info( TimeMsg( "Info: Predict important name-entity and keyword in summarization v4. 20% Context was upset. 3:7" ) )
    ...
    # 創建訓練資料 : MaxLen : 1024 ( OOM ) , 只能使用 Len : 532  ( MainNe + CL + SubNe + CL +Context )
    ...
    for epoch in range(mTrainEpoch):
        ...
        mBartModel.save_pretrained("Checkpoint_vINEPulsAnd37/point_{}".format(str(epoch)))
        ...
        for index , row in testingPd.iterrows() :
            tmpContext = NameEntitysHandler(row['name_entity_fix']) + '[CL]' + NameEntitysHandlerWithSize(row['ne_keyword']) + '[CL]' + row["context"]
            tmplabelZh = CsvSummaryZhHandler(row['zh_refs'])
            tmpInput = mBartTokenizer( tmpContext, max_length=532, return_tensors='pt' )
            tmpInput.to(mDevice)
